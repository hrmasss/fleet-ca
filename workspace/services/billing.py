...
def _price_for_plan(plan: str, renew_interval: str | None = None) -> str:
    ...
    if not price:
        ...
        price = os.getenv(fallback)
        if not price:
            raise serializers.ValidationError(
                f"Stripe price id not configured for plan '{plan}'. Set {key} or {fallback} in env."
            )
    return price
...
def create_checkout_session(*, user, workspace, plan: str) -> Dict[str, Any]:
    ...
    if not settings.STRIPE_SECRET_KEY:
        raise serializers.ValidationError("Stripe not configured.")
